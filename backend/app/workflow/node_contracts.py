...
NODE_CONTRACTS: dict[tuple[str, str], NodeContract] = {
    ...
    ("commander_agent", "search_public_references"): NodeContract(
        agent_id="commander_agent",
        action="search_public_references",
        tool_name="mcp.public-reference.search_wikimedia",
        node_type="tool",
        input_schema={"query": "customer public-reference query, maximum 140 characters"},
        output_schema={
            "query": "approved search query",
            "sources": "at most three fixed-origin public reference records",
            "warnings": "bounded provider warnings",
            "fact_boundary": "public reference clue only; not automatically verified facts",
        },
        state_writes=("mcp.public_reference.sources", "conversation.answer", "tool_calls"),
        ...
        evaluation_signals=("connection_enabled", "tool_catalog_verified", "source_origin_verified", "source_count"),
    ),
    ...
    ("data_agent", "plan_dataset_join"): NodeContract(
        agent_id="data_agent",
        action="plan_dataset_join",
        tool_name="agent.data_agent.plan_dataset_join",
        node_type="data",
        input_schema={
            "task_goal": "customer multi-dataset goal, maximum 1200 characters",
            "dataset_refs": "exactly two explicitly selected CSV/XLSX references",
            "left_dataset": "left-side selected dataset",
            "right_dataset": "right-side selected dataset",
            "left_key": "validated left join key",
            "right_key": "validated right join key",
            "join_type": "left|inner",
            "source_hashes": "hash for both selected datasets",
        },
        output_schema={
            "intent_version": "agentflow.data_join_intent.v1",
            "output_columns": "validated output column names",
            "output_row_count": "integer",
            "matched_row_count": "integer",
            "read_only": "true",
        },
        state_writes=("data.join.plan", "data.source_hashes"),
        required_permissions=("file_read",),
        failure_codes=("invalid_parameters", "data_file_unavailable", "data_join_failed"),
        evaluation_signals=("two_datasets_bound", "join_key_verified", "duplicate_key_checked", "preview_returned"),
    ),
    ("data_agent", "export_dataset_join"): NodeContract(
        agent_id="data_agent",
        action="export_dataset_join",
        tool_name="agent.data_agent.export_dataset_join",
        node_type="action",
        input_schema={
            "task_goal": "customer multi-dataset goal, maximum 1200 characters",
            "dataset_refs": "same two selected CSV/XLSX references used by preview",
            "left_key": "same validated left join key",
            "right_key": "same validated right join key",
            "join_type": "same left|inner connection type used by preview",
            "source_hashes": "hashes captured by planning step",
            "confirmed": "customer confirmation represented by Runtime permission decision",
        },
        output_schema={
            "delegated_task_id": "data join export task id",
            "agent_status": "completed|failed",
            "artifact": "verified agentflow-output CSV/XLSX copy",
            "verification": "output columns, row count and source hashes",
        },
        state_writes=("data.join.artifact", "artifacts", "delegations"),
        required_permissions=("file_read", "file_write"),
        failure_codes=("invalid_parameters", "data_file_unavailable", "data_join_failed", "artifact_verification_failed", "tool_timeout"),
        evaluation_signals=("two_datasets_bound", "join_readback_verified", "source_files_unchanged", "artifact_registered"),
    ),

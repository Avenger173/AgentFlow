...
> 版本：v1.8 | 建立日期：2026-09-16 | 最近更新：2026-09-23
>
> 状态：评测门槛已按“内部开发准入”和“用户发布准入”重新拆分。现有 `qwen-image-3.0-pro`、规划模型、失败语义、用量字段、媒体版本及导出证据已满足 `G0-DEV + L1-DEV`，允许进入 MM-2 AI 修图纵向闭环；这不等于 AI 修图可对用户开放。三份独立评审 CSV、完整图片质量集、费用核对、真实 Windows DPI 和客户端完整流程仍属于 G2/G3，当前均未通过。Lite Matting 与 SAM 是可选增强，不再阻塞不依赖它们的首个 AI 修图闭环。
>
...
- `G2`：AI 修图质量准入。要求固定质量集、独立人工复核、局部保护、多轮版本与冲突处理；可选模型仅在实际启用时 required。
- `G3`：用户发布准入。要求费用、权限、取消恢复、完整 Qt 流程、DPI、可访问性和原功能回归。

`G0-DEV`/`L1-DEV` 通过只允许继续开发，不改变 `runtime_ready` 和对外能力声明。历史无法追补的逐请求金额保持 unknown，不以重复付费重跑补材料。
...
| G2-G3 | 未运行 | 真实 AI 编辑尚未接入用户路径；质量独立复核、完整客户端流程、费用和发布回归均未通过 |

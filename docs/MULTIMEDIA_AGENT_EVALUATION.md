...
| QUAL-07A | ASR 句段时间包络 | 相对人工标注首尾包络，开发/留出集的 P95 绝对偏差 <= 500 ms、最大绝对偏差 <= 1500 ms | 这是转写时间戳的短媒体开发门槛；不是 EDL 精确裁剪的 200 ms 要求 |
...
CER/WER 使用编辑距离定义，报告替换、删除、插入及参考长度。数字和专名检查在规范化前单独完成，防止把错误数字规范化掉。`G4-ASR-DEV` 的时间偏差取人工参考句段与模型稳定句段的最早开始、最晚结束包络，避免因两端不同分句策略伪造逐句一一对应；它只证明字幕候选的时间基础，不能替代 EDL 边界、音画同步或人工可读性验收。无语音夹具不计算 CER/WER，按“不生成虚构转写”单独判定。
...
| G4-ASR-DEV | MM-4 前置 | `G4-ASR-DEV` 公开授权短媒体集、`MODEL-05` 的一次固定运行、每例 0/1 次调用、QUAL-07/07A 和既有转写任务恢复回归 | 只允许继续 EDL/字幕基础开发；不代表视频功能或正式 G4 |
| G4 | MM-4 | 前置 `G4-ASR-DEV`；VIDEO/LOCALIZE 计划与视频边界夹具；视频 profile 冻结；HARD-06/16、QUAL-06/07 的基础验证 | 不进入自然语言自动剪辑 |
...
| G4-ASR-DEV | 未运行 | 已完成受控短视频 -> WAV -> 一次转写 -> JSON Artifact 的真实探针，以及公开授权质量集/评分器的离线自测；但尚未冻结真实中英视频、人工参考标注或执行固定批量，不能计为内容质量通过 |
| G4 | 未运行 | 尚缺 `G4-ASR-DEV` 的真实质量结果，以及 EDL、字幕/导出、音画同步、长媒体策略和客户端流程；当前短视频任务闭环不等于视频功能 |
| G5 | 未运行 | 对话式剪辑尚未实现 |
| G6 | 未运行 | 视频翻译配音尚未实现 |

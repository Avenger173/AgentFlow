...
> 当前阶段以本节为准：阶段 5 内置 Agent MVP；知识库已完成 K4.1-K4.15、K5.1 本地检索短缓存、K5.2 Provider usage 基础可观测、K5.3 K4 任务指标聚合、K5.4 索引性能事实、K5.5 无变化索引快路径、K5.6 受控增量向量复用与 K5.7 上下文路由/预算边界，Commander 已完成 C5.1 的全库深度总结受控委派、C5.2 的父子状态镜像与 C5.3 的关联工作台入口。文档助手与数据工作台是可用的基础闭环，仍保留后续扩展空间；资料对照仍仅在知识库工作台启动。
...
- **唯一开放动作**：`data_agent.analyze_dataset` 已通过工程准入，且 `data_agent.runtime_ready=true` 只代表这一个动作。总指挥必须显式绑定一份已经导入并完成画像的 CSV/XLSX；没有数据、绑定多份数据，或客户要求导出/字段加工时，计划会澄清或转入数据工作台，不会伪造子任务。
...
> **当前唯一开发动作（2026-08-26）：C6.5.3 专业子任务模型审计。**为文档、数据和知识库子任务补齐“实际使用的脱敏模型路由与阶段”历史展示；不新增 Agent、不调用模型、不读取客户材料、不改写旧任务。完成判据、边界和离线回归见 `docs/COMMANDER_C6_PRODUCT_PLAN.md` 的 “C6.5.3”。本节其余内容是阶段历史记录，不应被误读为当前待办。
...
总指挥已确认且协议已部分落地，后续只随真实专业 Agent 的能力扩大路由；通用代码工坊不再开发，命令治理作为平台能力保留。数据工作台已统一使用“数据工作台”导航、渐变背景与渐进披露的独立结果阅读：D1 受控导入/画像，D2 本地预览，D3-D4.3A 可恢复异步交付，D5.1 建议，D5.2 PNG 看板，D5.3 字段加工新副本，以及 D5.4 总指挥单数据集只读分析均已实现。D5.4 不代表数据 Agent 的写入、导出或多数据集能力已经进入总指挥路线。

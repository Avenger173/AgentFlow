...
> **2026-09-24 多媒体助手 MM-4 语音转写协议底座：**已新增 `media_transcription -> qwen_audio / qwen-audio-3.1-asr-flash` 独立模型路由；它安全复用已保存的 Qwen DPAPI 密钥，但 Base URL、模型和审计配置可单独管理。Adapter 仅接收内存 WAV/MP3 字节（原始输入上限 `7 MiB`），以 Base64 直连；不足一分钟解析 JSON 终态，较长音频解析 SSE，并只接收稳定句/词时间戳。它不会上传本机路径或公共 OSS，也不会把媒体正文放进长期记忆。`verify_qwen_audio_transcription.py` 已覆盖请求边界、JSON/SSE 时间戳、usage/request ID、明确拒绝和未知结果不自动重试，`verify_model_configuration.py` 已覆盖路由与共享密钥解析。此前短 WAV Probe 证明协议端点可用；当前受控视频源与可恢复转写任务已在上一条状态记录完成真实闭环。字幕、EDL、长媒体策略与 Qt 页面尚未实现，`G4` 未运行。
...
> **2026-09-23 多媒体助手 G2 首轮真实质量运行：**已用 12 个公开来源冻结开发/留出集 `8/4`，每来源构造换背景、移除合成贴纸、中文价格改字各一项，共 36 项；固定 `media_image_edit -> qwen_image / qwen-image-3.0-pro`，每项最多一次且无自动重试。36/36 均返回、下载并回读为同尺寸 PNG，无限流、拒绝、未知或下载失败；随后离线最低效果回读确认换背景、移物、改字各 `12/12`，开发集 `24/24`、留出集 `12/12`，因此 G2-DEV 已满足。两份 36 行独立评审 CSV 仍为空，表示正式 G2 待复核，不是模型质量失败。
...
> **2026-09-11 Agent 记忆 MEM-7 自动验收：**当前已配置的 `kimi / kimi-k2.6` 在临时 SQLite 与合成夹具中完成八个记忆场景，17 次真实调用均返回 usage（输入 13,371、输出 4,704、总计 18,075 token）。工作状态、压缩连续性、异步恢复、全局偏好、项目隔离、改写检索、Dense 失效降级与关闭记忆均通过；生产数据库和运行偏好没有被修改。此结论不等同于客户路径已完成：Qt 仍需在桌面端现有 60 秒超时配置下执行会话切换、恢复、候选确认、关闭/删除和隔离检查，详见 `docs/AGENT_MEMORY_MEM7_ACCEPTANCE.md`。
...
> **2026-09-24 MM-4 公开夹具准备器：**新增 `prepare_media_transcription_quality_fixtures.py`，将已获授权的本地音频和参考文本以显式 FFmpeg/FFprobe 封装为黑底 MP4，并回读流、时长和 SHA-256，输出只允许位于 Git 忽略的 `data/` 目录。它需要计划中显式填写数据集来源/许可证/权利确认，拒绝重复源、路径逃逸、超过 `200 s` 的音频和未提供音视频流的输出；没有人工段落标注时只生成 `pending_human_review` 草案，不能成为可评分的 G4-ASR-DEV 质量集。开发机真实 FFmpeg/FFprobe 自测已通过，重复源负例已拒绝；未下载公共数据、未读取客户材料、未调用模型。Mozilla Data Collective 的公共目录已确认存在 CC0 中英文 ASR 候选，但正式下载需要独立 MDC API Key，不能复用 Qwen Key。

> **2026-09-24 MM-4 质量运行执行器：**新增 `run_media_transcription_quality.py`，默认只校验冻结 suite；仅显式 `--execute` 才会在全部 `8` 个夹具完成来源、哈希、音视频流与时长预检后，按顺序调用 `media_transcription`。每例调用上限为 `1` 且没有自动重试；遇到验证失败、Provider 拒绝、取消、未知结果或 Artifact 回读失败，会立即停止，剩余样本标记为 `not_started`，评分器据此返回未通过而非索要不存在的 Artifact。真实 `run.json`、转写 Artifact 与失败类别只写入忽略的证据目录；离线自测覆盖未知结果停批、失败样本不得伪造 Artifact、调用数与失败类别一致性，未调用模型。由于真实公开授权质量集尚未冻结，本轮固定 `8` 次批量尚未获确认，`G4-ASR-DEV` 和完整 G4 仍未通过。
...
> **2026-09-09 调度台公开资料表达与运行态修正：**固定 Wikimedia 公开资料检索的客户会话不再展示
> `dry-run`、Runtime、虚构的“情报检索 Agent”或多站检索承诺；规划阶段只说明固定来源、最多三条带链接参考和“回复开始继续”的真实边界。Qt 将“开始/确认”识别为当前已规划任务的确认，提交、实际 `queued/pending/running` 与终态统一驱动活动指示器，并保持在调度台展示检索过程与交付，不再自动跳转任务历史。LangGraph 仍仅为已拒绝准入的影子评估后端；LangChain 未接入客户路径，均不得作为项目已落地技术写入简历。
...
> **2026-09-07 至 2026-09-08 LGM5.1-LGM5.7 Commander 组合影子图、bridge、业务 Adapter、稳定委派回读、父协调器与试点准入：**`LangGraphCommanderCompositionShadowBackend` 只接收已经通过 C6.4 Native Runtime 准入的文档/数据/知识库只读组合计划。动态 invocation 图以步骤、动作、材料摘要和输入摘要哈希建立独立分支；一个分支失败不会取消其它独立分支，恢复同一 task 时只重派发失败/未开始分支，最终汇总只使用实际成功的受限摘要。专项夹具已验证“文档完成、数据首次失败、恢复后仅数据重试、完整汇总”，并检查 Graph SQLite checkpoint 不含原始目标、材料名或正文；同一三分支 C6.4 fixture 在 Native/Graph 两条路径上的完成/失败集合与 `partial` 汇总范围也完全一致。LGM5.3 的主库 `langgraph_runtime_bridges` 关联表和稳定调用键保证同一 Runtime 只能绑定同一图版本、线程与计划摘要，终态与已完成 invocation 不能回退。LGM5.4 的业务 Adapter 只能从主库复核已批准步骤，伪造 invocation 会在执行器前停止，且只把固定状态/数量回执交给图。LGM5.5 已让文档、数据预览和知识库问答 handoff 将同一 `delegation_call_id` 映射为稳定 child task ID；命中已完成的子任务快照后会直接回读而非重复调用，数据预览也新增对等回读接口。LGM5.6 的单父协调器让 Graph 并发分支只收集 Native 回执、由同一线程按计划顺序写回父任务步骤/产物/事件；部分完成保持 `blocked` 以便恢复，父 checkpoint 失败或已有子回执后的 Graph 异常均明确收束 bridge，只有尚未产生回执时才允许注入 Native fallback。LGM5.7 额外建立最终 `langgraph_composition_trial_admissions` 与候选 `langgraph_composition_trial_authorizations`：两者只保存计划/材料/模型摘要、基线与不透明审批引用。候选预授权须满足双重开发者开关和真实材料/模型授权，可在 Graph 创建前撤销；成对准备器从一份完成的 C6.4 dry-run 创建 Native 与候选 Runtime，但只在候选侧执行安全根步骤，专业调用仍保持 pending。候选目录/命令默认只展示脱敏任务身份，并要求 `--confirm-prepare` 才写入内部 Runtime 检查点。候选完成仍须通过调用/事件/产物/恢复对照及 10% 启动/内存基线，才会生成最终准入。观察器会从两条 Runtime 已保存的步骤、Tool、产物、事件和交付数量投影生成对照证据，避免调用方手填“已通过”。Graph/bridge/父 checkpoint 异常只返回 Native 重试路线；Graph 预读 checkpoint 初始化异常现同样会把 bridge 收束为 `failed`，不遗留假运行态。专项回归不调用真实模型、网络、MCP 或客户文件。一次已授权真实对照已完成，但独立资源测量超过当前门槛，最终准入已拒绝；未注册 API/Qt/Router 或改变客户路径，Native 仍是唯一客户执行路径。
...
- 已通过 `verify_commander_data_join_delivery.py`、既有数据交付/字段加工/图表/路由回归、Python 编译和 `git diff --check`；本轮使用临时数据与 mock，不调用真实模型或网络。R5.4D 已进入统一结果卡实现。
...
- 已修复调度台对明确 PPT 请求只返回“请前往文档助手”的问题：识别到制作意图后，自动打开独立、可移动的 PPT 创作窗口，沿用现有计划、导出和回读验证链，主页面不切换。
- 直出模式不再要求客户重复点击“开始执行”或确认同一份导出计划；默认使用内置版式和已配置模型的数据规划，仍只写入受控 `output/document_presentations`，外部图片和联网素材不会被静默启用。
- 主调度台不再显示占空间的内嵌“本次结果”卡；终态详情仍通过可移动结果窗口和任务历史查看。PPT 结果会回到当前会话，并带页数、文件名/相对路径和验证状态。
- 调度台活动指示器已覆盖 PPT 计划、导出和回读阶段。制作中持续旋转，完成/失败/关闭后台监听后都能正确停止或回传，不再出现“文字不动但任务已结束”的歧义。
- 同一会话下的“你倒是制作啊”“开始制作”“继续生成”等续话会复用当前 PPT 目标，制作中不重复发起任务，失败后可重新打开同一目标。
...
- 新增 MCP 无副作用契约，固定规范化 server/tool 名称、传输候选与稳定错误码；随后 LGM1 已在项目内测试服务上完成受控 schema/结果 Guard、短会话和审计回归，详见上方 LGM1 记录。
...
- 下一步：确认 LGM2 的首个真实只读客户场景。仍不连接真实外部服务、不调用真实模型、不读取客户材料；LangGraph K4 影子迁移仍是 LGM3/LGM4 的后续工作。

## 2026-09-03 LangGraph、LangChain 与 MCP 平台集成计划

- 用户已确认三项技术进入 AgentFlow 的长期方向；LGM0/LGM1 已完成依赖、边界与测试 Gateway 验证，**仍未连接真实 MCP server、切换客户任务或宣称框架能力已经交付**。
...
- 下一步固定为 LGM2 的场景确认：先确定一个真实只读客户价值，再讨论真实 MCP Server、配置和权限闭环；不调用真实模型、不读取客户材料、不连接真实外部服务。

...
- 已通过 `verify_delivery_card.py`，验证结果卡版本、终态、产物能力标记和路径脱敏。Qt 调度台已接入默认收起的结果卡：规划受理后显示状态，任务进入终态后自动补读同一卡，并渲染结论、有限事实、警告、产物和下一步动作；旧任务响应按 task_id 丢弃，完整过程仍留在历史页。图片缩略图、表格摘要以及知识库/文档专用事实卡仍是后续扩展。

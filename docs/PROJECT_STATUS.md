...
> **2026-09-07 LGM5.1-LGM5.4 Commander 组合影子图、bridge 与业务 Adapter 准入：**`LangGraphCommanderCompositionShadowBackend` 只接收已经通过 C6.4 Native Runtime 准入的文档/数据/知识库只读组合计划。动态 invocation 图以步骤、动作、材料摘要和输入摘要哈希建立独立分支；一个分支失败不会取消其它独立分支，恢复同一 task 时只重派发失败/未开始分支，最终汇总只使用实际成功的受限摘要。专项夹具已验证“文档完成、数据首次失败、恢复后仅数据重试、完整汇总”，并检查 Graph SQLite checkpoint 不含原始目标、材料名或正文；同一三分支 C6.4 fixture 在 Native/Graph 两条路径上的完成/失败集合与 `partial` 汇总范围也完全一致。LGM5.3 的主库 `langgraph_runtime_bridges` 关联表和稳定调用键保证同一 Runtime 只能绑定同一图版本、线程与计划摘要，终态与已完成 invocation 不能回退。LGM5.4 的业务 Adapter 只能从主库复核已批准步骤，伪造 invocation 会在执行器前停止，且只把固定状态/数量回执交给图。尚未复用真实专业 Runtime executor、注册 API/Qt/Router 或改变客户路径；Native 仍是唯一客户执行路径，下一步才完成 executor、事件/交付回读和故障回退夹具。

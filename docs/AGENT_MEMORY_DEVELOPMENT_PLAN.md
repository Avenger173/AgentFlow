...
> 状态：实施中（MEM-0 至 MEM-6 已完成；MEM-7 真实模型自动验收已通过，Qt 客户路径人工验收待完成；BM25 已准入，Hybrid 暂不准入）
>
> 建立日期：2026-09-10
>
...
- `verify_memory_real_provider.py` 已使用当前已配置的 `kimi / kimi-k2.6` 路由完成八个合成场景；17 次 Provider 请求均返回 usage，输入/输出/总 token 分别为 13,371 / 4,704 / 18,075。验证使用临时 SQLite 与运行偏好文件，不修改生产数据库或生产偏好。
- 自动证据同时核验真实模型非空回复、同请求的 ContextEnvelope/Commander 工作流快照、Working State、检索观测与 `last_used_at`。它不会将模型对合成标记的逐字复述误当作注入证明；客户可见表述由 Qt 人工清单复核。
- 验证进程采用 120 秒超时和 21 秒最小请求间隔，以适配已观察到的 3 RPM 限制。桌面端当前仍为 60 秒超时，尚未完成 Qt 人工清单，故 MEM-7 不可标记为完全通过。

### MEM-7 补充：情景记忆只读回顾与动作边界

**问题与决策**

- 会话原文已按项目范围完整脱敏归档，但此前只有会话列表和逐页正文查看入口；用户在调度台问“我叫你生成过什么内容的 PPT”时，规则层会把 `PPT` 错判为新的制作意图。
- 新增显式历史回顾通路：仅对含有回顾信号和问题信号的提问生效，在 Commander 规划和模型调用前查询同一 `project_scope` 的已归档用户请求，最多返回五条短摘录。
- 回顾响应是独立的 `history_recall` 模式，没有 WorkflowPlan、dry-run、工具调用、工作台跳转或文件写入；它不会被当作普通会话上下文自动注入，也不会更新长期记忆的 `last_used_at`。
- 情景记忆与语义记忆继续分层：会话归档是用户主动回顾的经历记录；已确认的 `user_preference` 才是可见、可编辑、可关闭的用户画像来源。程序性记忆仍由 `SKILL.md`、Agent Definition 和 Workflow 规则承担。

**出口条件**

- 相同提问在 mock 和 LLM 路径都不生成 PPT 计划或文件。
- 同一范围内可回顾已保存请求，跨项目范围泄漏数为 0。
- 明确的新 PPT 创建请求仍进入既有 `open_presentation_studio` 路由。
- 自动验证见 `docs/AGENT_MEMORY_EPISODIC_RECALL_ACCEPTANCE.md`；该补充不替代 MEM-7 尚未完成的 Qt 人工验收。

...
#### LGM4.2 实施记录（2026-09-04）

- 影子后端现从既有 Native K4 `WorkflowRun.metrics` 读取步骤、已完成/失败数、端到端耗时、
  实际 Provider 请求数与重试数，并附加 Graph 墙钟耗时、已保存 Graph 节点数。它不计算或
  猜测 token、费用、正文大小或缓存命中。
- K4 Map/Reduce 的既有 progress callback 已归一化为 Harness 的无正文阶段事件；客户真正接入
  时可复用现有 Event Projection，而不用把章节名、框架节点名、模型消息或 SDK 对象暴露给 Qt。
  影子回归确认开始、处理中与完成事件顺序，以及 Native/Graph 的 Map/Reduce 调用次数一致。
...
- 2026-09-08 已在开发者显式双授权下，以一份冻结的只读“知识库回答 + 数据预览 + 总结”组合计划完成
  Native 基线与 Graph 候选的真实对照。两侧的专业步骤、Tool 形态、受控产物、客户交付投影均一致；
  Graph 首次仅有一个独立分支失败，恢复时只重放该分支，已完成分支没有再次派发或消耗模型调用。
- 真实运行暴露并修复四个接缝：组合计划向知识库子任务传递了跨领域总目标、同一 Kimi Provider 的两条
  组合调用并发不稳定、Graph 稳定 delegation key 不满足知识库子任务 ID 契约、协调器遗漏 synthesis

...
- 状态：**自动化离线发行验收已完成；人工 UI 验收说明已就绪；真实模型/远端 MCP 仍需逐次明确授权。**
- `benchmark_directory_release.py` 会独立重复启动候选 Qt 客户端，聚合“客户端到随包后端健康”、
  Qt + 后端 RSS、并发只读状态请求、关闭到端口释放的中位数；全部使用临时用户目录和 mock，
  不读取 `.env`、客户材料、模型配置或真实 outputs。
- 当前本机构建候选的 3 轮中位数为：就绪 `6664ms`、运行时 RSS `323.8MiB`、关闭/端口释放
  `2149ms`、运行时进程数 `2`，以及 12 路受控只读状态请求的最慢响应 `5193ms`。这是当前构建机的
  诊断基线而不是跨电脑 SLO；后续新增默认运行时必须在同机、同候选上与它比较。
- `verify_directory_client_smoke.py` 现在会同时核对运行时树只存在 `AgentFlow.exe` 与
  `AgentFlowBackend.exe`，默认关闭时没有 Node/MCP 子进程，退出后 `8765` 与该进程树均已释放。
- `verify_knowledge_deep_task_map.py` 已回归 Map/Reduce 长任务的协作式取消、checkpoint 和部分结果边界；
  `verify_data_charts.py` 已回归异步交付取消后无 artifact 遗留。二者均为固定模型/合成数据的本地测试，
  不是额外消耗模型额度的探针。
- 每个新增运行时与无可选能力基线比较；超过当前 10% 默认门槛的组件继续保持关闭，不靠提升阈值放行。
- 真实模型或远程 MCP 验收只能在明确授权后进行，记录有限聚合指标与脱敏状态，不记录客户正文或 Key。

出口：**已达到自动化离线出口。**发行候选的启动、关闭、离线、损坏依赖和恢复都有自动回归；
人工 UI 验收按 [LGM7_RELEASE_MANUAL_ACCEPTANCE.md](LGM7_RELEASE_MANUAL_ACCEPTANCE.md) 执行。
真实 Provider 或外部 MCP 的验收不属于默认发行，也不能借此出口自动发生。

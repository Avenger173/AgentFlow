...
- 验证 MCP 默认停用、配置损坏、远端 Server 不可达、Node 缺失、embedding/OCR 模型未下载、后端文件损坏、
  端口被占用等场景的客户提示与 Native 回退。
- 验证可选 Node Harness 携带时不读取客户工作区 `.env`，禁用后不启动子进程；MCP 连接备份、迁移、禁用与
  密钥清理只作用于连接配置和专属状态。
- 验证旧 SQLite 任务、Native checkpoint、被拒绝的 LangGraph 试点记录与旧 artifact 的回读兼容；不迁移或
  删除客户成果。

出口：基础 Native 能力可在离线与任一可选依赖缺失条件下运行；客户能理解故障、恢复入口与数据影响。

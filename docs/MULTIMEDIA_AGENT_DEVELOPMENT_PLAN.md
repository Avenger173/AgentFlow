...
>
> 当前结论：多媒体助手采用“模型负责理解与生成、成熟媒体工具负责精确执行、AgentFlow 负责调度与可靠交付”的路线。现有图片工作区保留为工程底座并停止横向扩建；MM-2 后端纵向闭环已完成离线验证，下一任务是接入最小 Qt 自然语言入口，而不是继续建设通用图片编辑器。
>
...
| MM-2 AI 修图纵向闭环 | 调度台/工作区提交图片与提示词，调用 `media_image_edit`，写入新 revision，预览、继续修改、撤销和导出 | `G2`：固定真实样本质量、局部保护、多轮版本和未知结果处理通过 | 后端链路已完成：受控字节输入、模型/下载失败分类、版本提交、取消与重启对账均有离线验证；Qt 入口与真实质量验收待做 |
...
MM-2 后端已按下列前四项完成；下一段对话从第 5 项继续：

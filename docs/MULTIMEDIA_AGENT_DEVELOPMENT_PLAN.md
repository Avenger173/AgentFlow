...
>
> 当前结论：多媒体助手采用“模型负责理解与生成、成熟媒体工具负责精确执行、AgentFlow 负责调度与可靠交付”的路线。现有图片工作区保留为工程底座并停止横向扩建；MM-2 已接入最小 Qt 自然语言入口，并以合成夹具完成一次模型到受控 revision 的真实闭环验证，下一步是冻结 G2 质量评测集，而不是继续建设通用图片编辑器。
>
...
7. 已落地 G2 质量集离线数据契约：`verify_media_g2_quality_suite.py` 会拒绝非 `12` 来源、开发/留出集非 `8/4`、非 `36` 任务、跨 split 复用来源、类别配额失衡或评审协议缺失的 suite。下一步冻结真实公开/授权图片开发集与留出集，并执行局部消除、换背景、中文文字编辑的固定样本；不以本次合成夹具闭环替代内容质量结论。

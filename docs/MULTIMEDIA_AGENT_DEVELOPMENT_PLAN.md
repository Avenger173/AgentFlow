...
>
> 当前结论：多媒体助手采用“模型负责理解与生成、成熟媒体工具负责精确执行、AgentFlow 负责调度与可靠交付”的路线。现有图片工作区保留为工程底座并停止横向扩建；MM-2 已接入最小 Qt 自然语言入口，并完成一轮固定 G2 质量集的真实模型调用与盲评包生成。当前等待独立人工盲评，不继续建设通用图片编辑器。
>
...
| MM-2 AI 修图纵向闭环 | 调度台/工作区提交图片与提示词，调用 `media_image_edit`，写入新 revision，预览、继续修改、撤销和导出 | `G2`：固定真实样本质量、局部保护、多轮版本和未知结果处理通过 | Qt 图片工作区已接入指令、状态和异步结果轮询；后端受控字节输入、失败分类、版本提交、取消与重启对账均有离线验证。一次合成夹具真实调用已完成 revision 回读；G2 质量集、独立复核和完整客户端流程仍待做 |
...
7. 已落地 G2 质量集离线数据契约：`verify_media_g2_quality_suite.py` 会拒绝非 `12` 来源、开发/留出集非 `8/4`、非 `36` 任务、跨 split 复用来源、类别配额失衡或评审协议缺失的 suite。2026-09-23 已用 12 个公开来源冻结真实 suite，并通过 `probe_qwen_image_g2_quality.py` 对 `qwen_image / qwen-image-3.0-pro` 执行 36 项各一次的真实调用；全部回读为同尺寸 PNG，未发生自动重试。`verify_media_g2_review_packet.py` 已确认 36 张无模型名的盲评卡片与两份空白评审表一致。下一步是由两位独立评审填写评分并复核分差；不以本次连通性或开发者抽查替代内容质量结论。

...
8. 已新增 `verify_media_transcription_quality_suite.py` 与 `evaluate_media_transcription_quality.py`，建立 `G4-ASR-DEV` 的机器契约：仅接受 `8` 段人工权利确认的公开授权源视频（开发/留出 `5/3`、中英均覆盖、合计至少 `8 min`），每段均需 SHA-256、人工校对文本及单调句段时间标注。当前 `16 kHz` 单声道 WAV 的 `7 MiB` 输入上限约束每段派生音频至 `<=200 s`；程序生成夹具只能验证工程协议，明确拒绝计入内容质量。评分器只回读已验证的转写 Artifact，强制每例 `0/1` 次 Provider 调用，按开发/留出集分别计算中文 CER `<=15%`、英文 WER `<=20%`，并记录转写句段首尾包络的 P95 `<=500 ms`、最大 `<=1500 ms` 偏差；报告不输出转写正文。
9. 已新增 `prepare_media_transcription_quality_fixtures.py`：它不下载大型语料、不读取用户项目媒体、也不调用模型；只将已取得许可的本地音频与参考文本按计划封装为黑底 MP4，回读音视频流/时长，写入哈希和候选 suite。输出强制位于忽略的 `data/` 下，避免公开源材料、转写文本或评测证据被意外提交。没有人工段落标注时，脚本只写 `pending_human_review`，质量集仍保持未就绪；重复源音频、非 HTTPS 来源/许可证和超过 `200 s` 的样本均会拒绝。其真实 FFmpeg/FFprobe 自测已通过，且复用源负例被拒绝。当前尚未收集实际公开授权中英视频或调用模型，`G4-ASR-DEV` 与完整 `G4` 都不能标通过。下一步是用独立 MDC API Key 或其它已获授权来源准备真实夹具、人工复核文本/时标，先跑离线契约和既有恢复回归，再由用户确认一次固定批量真实运行；不提前扩展字幕、翻译、配音或剪辑界面。
10. 已新增 `run_media_transcription_quality.py`，把“冻结质量集 -> 每例一次真实 Runtime -> Artifact 回读 -> `run.json` 离线评分”接成受控批量。默认仅校验 suite，`--execute` 才允许模型调用；在第一笔调用前必须完成全量来源/哈希/音视频流/时长预检。批次严格顺序执行，任何未完成结果都会停止后续提交并将剩余夹具写为 `not_started`，不重放未知请求；`run.json` 仅记录路由、Provider/模型、失败类别、调用数和 Artifact 哈希，证据文件仅落到忽略目录。执行器与评分器均已覆盖未知结果、不完整批次和伪造调用数的离线负例；未收集已授权质量集、未获得本轮固定批量确认前，真实模型调用数仍为 `0`，`G4-ASR-DEV` 仍未通过。
